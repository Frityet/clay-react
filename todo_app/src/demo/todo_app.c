...
static const TodoTheme k_theme_dark = {
    .background = { 14, 16, 20, 255 },
    .surface = { 24, 27, 32, 255 },
    .surface_alt = { 32, 36, 42, 255 },
    .text = { 226, 231, 238, 255 },
    .text_muted = { 148, 158, 170, 255 },
    .accent = { 96, 165, 250, 255 },
    .accent_soft = { 30, 64, 102, 255 },
...
static bool s_compact_layout = false;
static Clay_Dimensions s_viewport_dimensions = { 1024.0f, 768.0f };

const TodoTheme *TodoAppTheme(void) {
    return s_dark_mode ? &k_theme_dark : &k_theme;
}

void TodoAppSetViewport(Clay_Dimensions dimensions) {
    s_viewport_dimensions = dimensions;
}
...
typedef struct TodoItem {
    ...
    uint8_t tag;
    CR_TextInputState * $nullable input;
} TodoItem;
...
static const char *_Nonnull k_priority_labels[] = { "Low", "Medium", "High" };
static const char *_Nonnull k_filter_labels[] = { "All", "Active", "Done" };
static const char *_Nonnull k_sort_labels[] = { "Created", "Priority", "Alpha" };
static const char *_Nonnull k_tags[] = { "Work", "Home", "Study", "Errand", "Health" };
...
static float todo_clamp(float value, float min_value, float max_value) {
    if (value < min_value) return min_value;
    if (value > max_value) return max_value;
    return value;
}

static float todo_fpx(float value) {
    return value * s_ui_scale;
}

static uint16_t todo_px(float value) {
    if (value <= 0.0f) return 0;
    float scaled = value * s_ui_scale;
...
static TextConfig text_style_title(const TodoTheme *theme) {
    return (TextConfig){
        .font_id = 0,
        .font_size = todo_px(32),
        .line_height = todo_px(40),
        .color = theme->text,
    };
}

static TextConfig text_style_body(const TodoTheme *theme) {
    return (TextConfig){
        .font_id = 0,
        .font_size = todo_px(16),
        .line_height = todo_px(24),
        .color = theme->text,
    };
}

static TextConfig text_style_muted(const TodoTheme *theme) {
    return (TextConfig){
        .font_id = 0,
        .font_size = todo_px(14),
        .line_height = todo_px(20),
...
static TextConfig text_style_chip(Clay_Color color) {
    return (TextConfig){
        .font_id = 0,
        .font_size = todo_px(12),
        .line_height = todo_px(16),
        .color = color,
    };
}
...
static Clay_Color todo_priority_color(TodoPriority priority, const TodoTheme *theme) {
    switch (priority) {
...
static Clay_Color todo_border_color(const TodoTheme *theme, bool is_dark) {
    return is_dark ? $alpha(theme->text_muted, 110) : $gray(220);
}

static Clay_Color todo_panel_border_color(const TodoTheme *theme, bool is_dark) {
    return is_dark ? $alpha(theme->text_muted, 140) : $gray(230);
}
...
static void TodoChip(const char *label, Clay_Color bg, Clay_Color text) {
    ...
    Box((BoxParams){
        .style = {
            .layout = { .padding = $pad_lrtb(todo_px(8), todo_px(8), todo_px(3), todo_px(3)) },
            .background = bg,
...
$component(TodoRow, TodoRowProps) {
    ...
    Clay_Color pr_color = todo_priority_color(item->priority, theme);
    ...
    Row((BoxParams) {
        .style = {
            .layout = {
                .sizing = { .width = $grow(0) },
                .padding = $pad_lrtb(todo_px(14), todo_px(14), todo_px(12), todo_px(12)),
                .childGap = todo_px(12),
            },
            .background = row_bg,
            .background_hover = $alpha(theme->accent, 20),
            .corner_radius = $radius(todo_fpx(10)),
        },
    }, ^{
        Checkbox((CheckboxParams) {
            ...
            .checked = done,
            .on_toggle = ^{
                if (!props->state || !props->set_state || props->index < 0 || props->index >= props->state->count) {
                    return;
                }
                TodoState next = *props->state;
                next.items[props->index].done = !next.items[props->index].done;
                next.version++;
                props->set_state(next);
            ...
            if (props->is_editing) {
                TextInput((TextInputParams){
                    ...
                    .style = {
                        .layout = {
                            .sizing = { .width = $grow(0) },
                            .padding = $pad_lrtb(todo_px(10), todo_px(10), todo_px(8), todo_px(8)),
                        },
                        .background = theme->surface,
                        .corner_radius = $radius(todo_fpx(6)),
                        .border = (Clay_BorderElementConfig){
                            .width = $border_outside(todo_px(1)),
                            .color = todo_border_color(theme, props->is_dark),
                        },
                        .has_border = true,
                    },
                    .focus_border = (Clay_BorderElementConfig){
                        .width = $border_outside(todo_px(2)),
                        .color = theme->accent,
                    },
                    .has_focus_border = true,
                    .text = text_style_body(theme),
                });
            } else {
                Clickable((ClickableParams){
                    .id = cr_idi("TodoTitle", (uint32_t)item->id),
                    .on_click = ^{
                        if (!props->state || !props->set_state || !item) {
                            return;
                        }
                        TodoState next = *props->state;
                    ...
                    .style = {
                        .layout = { .padding = $pad_lrtb(todo_px(6), todo_px(6), todo_px(4), todo_px(4)) },
                        .corner_radius = $radius(todo_fpx(6)),
                    },
                }, ^{
                    Text((TextParams){
                        .text = item->title,
                        .style = done ? text_style_muted(theme) : text_style_body(theme),
                    });
                });
            }
        ...
        Row((BoxParams){
            ...
            if (props->is_editing) {
                Button((ButtonParams){
                    ...
                    .on_click = ^{
                        ...
                        if (item->input && item->input->length > 0) {
                            strncpy(next.items[props->index].title, item->input->buffer, TITLE_MAX - 1);
                            next.items[props->index].title[TITLE_MAX - 1] = '\0';
                        }
                        next.editing_id = -1;
                        next.version++;
                        props->set_state(next);
                    ...
                    .style = {
                        .layout = {
                            .padding = $pad_lrtb(todo_px(10), todo_px(10), todo_px(6), todo_px(6)),
                    ...
                    .text = (TextConfig){ .font_size = todo_px(13), .color = $WHITE },
                }, NULL);
                ...
                IconButton((IconButtonParams){
                    ...
                    .icon = "x",
                    .on_click = ^{
                        if (!props->state || !props->set_state) {
                            return;
                        }
                        TodoState next = *props->state;
                        next.editing_id = -1;
                    ...
                    .text = (TextConfig){ .font_size = todo_px(12), .color = $WHITE },
                }, NULL);
            } else {
                IconButton((IconButtonParams){
                    ...
                    .on_click = ^{
                        ...
                        TodoState next = *props->state;
                        next.items[props->index].pinned = !next.items[props->index].pinned;
                        next.version++;
                        props->set_state(next);
                        if (confirm) confirm->set(false);
                    },
                    .style = {
                        .layout = {
                            .sizing = { .width = $fixed(todo_fpx(46)), .height = $fixed(todo_fpx(28)) },
                            .childAlignment = { .x = CLAY_ALIGN_X_CENTER, .y = CLAY_ALIGN_Y_CENTER },
                        },
                        .background = $alpha(theme->accent, 50),
                        .background_hover = theme->accent,
                    },
                    .text = (TextConfig){ .font_size = todo_px(11), .color = $WHITE },
                }, NULL);
                ...
                if (confirm->get()) {
                    ...
                    .on_click = ^{
                        ...
                        TodoState next = *props->state;
                        TodoItem *next_item = &next.items[props->index];
                        int deleted_id = next_item->id;

                        if (_cr_focused_input == next_item->input) {
                            _cr_unfocus_input();
                        }
                        if (next_item->input) {
                            free(next_item->input->buffer);
                            free(next_item->input);
                        }

                        for (int i = props->index; i < next.count - 1; i++) {
...
$component(TodoAppView) {
    ...
    auto dark_mode = $use_state((bool)false);
    bool is_dark = dark_mode && dark_mode->get();
    s_dark_mode = is_dark;
    const TodoTheme *theme = is_dark ? &k_theme_dark : &k_theme;
    float viewport_width = s_viewport_dimensions.width;
    float viewport_height = s_viewport_dimensions.height;
    float min_side = viewport_width < viewport_height ? viewport_width : viewport_height;
    float root_padding = min_side > 0.0f ? min_side * 0.05f : 40.0f;
    if (root_padding < 16.0f) root_padding = 16.0f;
    if (root_padding > 40.0f) root_padding = 40.0f;
    float content_width = viewport_width > 0.0f ? viewport_width - root_padding * 2.0f : 760.0f;
    s_ui_scale = todo_clamp(content_width / 760.0f, 0.8f, 1.0f);
    s_compact_layout = content_width < 680.0f;
    ...
    TodoState *state_ptr = state->ptr;
    TodoStateSetter set_state = state->set;
    ...
    VoidBlock add_todo = $use_callback(^{
        if (!state_ptr || !set_state || !new_input) {
            return;
        }
        if (new_input->length == 0 || state_ptr->count >= MAX_TODOS) {
            return;
        }

        TodoState next = *state_ptr;
        ...
        strncpy(item->title, new_input->buffer, TITLE_MAX - 1);
        item->title[TITLE_MAX - 1] = '\0';
        item->input = _cr_alloc_text_input(TITLE_MAX);
        _cr_text_input_set_text(item->input, item->title);
        ...
        _cr_text_input_set_text(new_input, "");
        _cr_unfocus_input();
    }, $deps_once());

    VoidBlock clear_done = $use_callback(^{
        if (!state_ptr || !set_state) {
            return;
        }
    ...
    VoidBlock render_filter_buttons = ^{
        for (int i = 0; i < 3; i++) {
            ...
            Button((ButtonParams){
                ...
                .style = {
                    .layout = {
                        .padding = $pad_lrtb(todo_px(12), todo_px(12), todo_px(6), todo_px(6)),
                        .childAlignment = { .x = CLAY_ALIGN_X_CENTER, .y = CLAY_ALIGN_Y_CENTER },
                    },
                    .background = active ? theme->accent_soft : theme->surface_alt,
                    .background_hover = active ? theme->accent : $alpha(theme->accent_soft, 200),
                    .corner_radius = $radius(999),
                },
                .text = (TextConfig){
                    .font_size = todo_px(13),
                    .line_height = todo_px(16),
    ...
    VoidBlock render_sort_button = ^{
        Button((ButtonParams){
            ...
            .style = {
                ...
                .background_hover = theme->accent_soft,
                .corner_radius = $radius(999),
            },
            .text = (TextConfig){ .font_size = todo_px(13), .color = theme->text_muted },
        }, NULL);
    };

    VoidBlock render_search_input = ^{
        TextInput((TextInputParams){
            .id = search_input_id,
            .state = search_input,
            .placeholder = "Search tasks",
            .text = text_style_body(theme),
            .placeholder_text = text_style_muted(theme),
            .style = {
                .layout = {
                    .sizing = { .width = $grow(0), .height = $fit(.min = todo_fpx(40)) },
                    .padding = $pad_lrtb(todo_px(12), todo_px(12), todo_px(10), todo_px(10)),
                },
                .background = theme->surface_alt,
                .corner_radius = $radius(todo_fpx(8)),
                .border = (Clay_BorderElementConfig){
                    .width = $border_outside(todo_px(1)),
                    .color = todo_border_color(theme, is_dark),
                },
                .has_border = true,
            },
        });
    ...
    VoidBlock render_new_input = ^{
        TextInput((TextInputParams){
            ...
            .style = {
                .layout = {
                    .sizing = { .width = $grow(0), .height = $fit(.min = todo_fpx(44)) },
                    .padding = $pad_lrtb(todo_px(12), todo_px(12), todo_px(10), todo_px(10)),
                },
                .background = theme->surface_alt,
                .corner_radius = $radius(todo_fpx(8)),
    ...
    VoidBlock render_add_button = ^{
        Button((ButtonParams){
            ...
            .style = {
                .layout = {
                    .padding = $pad_lrtb(todo_px(16), todo_px(16), todo_px(10), todo_px(10)),
                    .childAlignment = { .x = CLAY_ALIGN_X_CENTER, .y = CLAY_ALIGN_Y_CENTER },
                },
                .background = theme->accent,
                .background_hover = $alpha(theme->accent, 220),
            ...
            .text = (TextConfig){ .font_size = todo_px(15), .color = $WHITE },
        }, NULL);
    };
    ...
    Column((BoxParams){
        ...
        .style = {
            .layout = {
                .sizing = { .width = $grow(0, 760), .height = $grow(0) },
                .padding = $pad_lrtb(todo_px(32), todo_px(32), todo_px(28), todo_px(28)),
                .childGap = todo_px(20),
            },
            .background = theme->surface,
            .corner_radius = $radius(todo_fpx(18)),
            .border = (Clay_BorderElementConfig){
                .width = $border_outside(todo_px(1)),
                .color = todo_panel_border_color(theme, is_dark),
            },
            .has_border = true,
        },
    }, ^{
        if (s_compact_layout) {
            Column((BoxParams){ .style.layout.childGap = todo_px(12) }, ^{
                Column((BoxParams){ .style.layout.childGap = todo_px(4) }, ^{
                    Text((TextParams){
                        .text = "Todo Atlas",
                        .style = text_style_title(theme),
                    });
                    Text((TextParams){
                ...
                Column((BoxParams){ .style.layout.childGap = todo_px(6) }, ^{
                    Textf((TextParams){ .style = text_style_body(theme) },
                        "Active %d | Done %d",
                        stats.active,
                        stats.done);
                    Textf((TextParams){ .style = text_style_muted(theme) },
                        "Pinned %d | Total %d", stats.pinned, stats.total);
                    Button((ButtonParams){
                        .id = cr_id("TodoDarkMode"),
                        .label = is_dark ? "Light mode" : "Dark mode",
                        .on_click = ^{
                            if (dark_mode) dark_mode->set(!dark_mode->get());
                        },
                        .style = {
                            .layout = {
                                .padding = $pad_lrtb(todo_px(10), todo_px(10), todo_px(4), todo_px(4)),
                                .childAlignment = { .x = CLAY_ALIGN_X_CENTER, .y = CLAY_ALIGN_Y_CENTER },
                            },
                            .background = is_dark ? theme->surface_alt : theme->accent_soft,
                            .background_hover = is_dark ? theme->accent_soft : theme->accent,
                            .corner_radius = $radius(999),
                        },
                        .text = (TextConfig){ .font_size = todo_px(12), .color = theme->text },
                    }, NULL);
                });
            });
        } else {
            Row((BoxParams){ .style.layout.childGap = todo_px(16) }, ^{
                Column((BoxParams){ .style.layout.childGap = todo_px(4) }, ^{
                    Text((TextParams){
                        .text = "Todo Atlas",
                        .style = text_style_title(theme),
        ...
        Box((BoxParams){
            .style = {
                .layout.sizing = { .width = $grow(0), .height = $fixed(todo_fpx(10)) },
            ...
            Box((BoxParams){
                .style = {
                    .layout.sizing = { .width = $percent(progress), .height = $fixed(todo_fpx(10)) },
                    .background = theme->accent,
                    .corner_radius = $radius(999),
                },
            }, NULL);
        ...
        if (s_compact_layout) {
            Column((BoxParams){ .style.layout.childGap = todo_px(10) }, ^{
                render_search_input();
                Row((BoxParams){ .style.layout.childGap = todo_px(8) }, ^{
                    render_filter_buttons();
                });
                render_sort_button();
            });
        } else {
            Row((BoxParams){ .style.layout.childGap = todo_px(12) }, ^{
                render_search_input();
                Row((BoxParams){ .style.layout.childGap = todo_px(8) }, ^{
                    render_filter_buttons();
                    render_sort_button();
        ...
        Column((BoxParams){
            ...
            if (s_compact_layout) {
                Column((BoxParams){
                    .style = { .layout = { .childGap = todo_px(6) } },
                }, ^{
                    Text((TextParams){ .text = "Priority", .style = text_style_muted(theme) });
                    Row((BoxParams){
                        .style = { .layout = { .childGap = todo_px(8) } },
                    }, ^{
                        for (int i = 0; i < 3; i++) {
                            bool active = state->ptr->draft_priority == (TodoPriority)i;
                            Clay_Color pr = todo_priority_color((TodoPriority)i, theme);
                            Button((ButtonParams){
                                ...
                                .style = {
                                    ...
                                    .background = active ? pr : theme->surface_alt,
                                ...
                                .text = (TextConfig){
                                    .font_size = todo_px(12),
                                    .line_height = todo_px(16),
                                    .color = active ? $WHITE : theme->text_muted,
                                },
                            }, NULL);
                        }
                    });
                ...
                Column((BoxParams){
                    ...
                    Text((TextParams){ .text = "Tag", .style = text_style_muted(theme) });
                    Row((BoxParams){
                        .style = { .layout = { .childGap = todo_px(8) } },
                    }, ^{
                        for (int i = 0; i < todo_tag_count() && i < 3; i++) {
                            ...
                            Button((ButtonParams){
                                ...
                                .style = {
                                    ...
                                    .background = active ? tag : theme->surface_alt,
                    ...
                    if (todo_tag_count() > 3) {
                        Row((BoxParams){
                            ...
                            for (int i = 3; i < todo_tag_count(); i++) {
                                bool active = state->ptr->draft_tag == (uint8_t)i;
                                Clay_Color tag = k_tag_colors[i];
                                Button((ButtonParams){
                                    .id = cr_idi("TodoTag", (uint32_t)i),
        ...
        Column((BoxParams){
            ...
            if (visible_count == 0) {
                Center((BoxParams){
                    .style = {
                        .layout = { .padding = $pad_lrtb(todo_px(24), todo_px(24), todo_px(18), todo_px(18)) },
                    },
                }, ^{
                    Text((TextParams){
        ...
        Row((BoxParams){
            ...
            Row((BoxParams){
                ...
                Text((TextParams){ .text = "Show done", .style = text_style_muted(theme) });
            });
            ...
            Button((ButtonParams){
                ...
                .style = {
                    ...
                    .background = $alpha(theme->danger, 80),
                    .background_hover = theme->danger,
                    .corner_radius = $radius(todo_fpx(8)),
                },
                .text = (TextConfig){ .font_size = todo_px(13), .color = $WHITE },
